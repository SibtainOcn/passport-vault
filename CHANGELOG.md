...
- Smart Updater & Automated Deployment Pipeline: Updated `UPDATE.bat` and `scripts/updater.ps1` with automatic git pull, multi-fallback Python interpreter detection (supporting both `.venv` and global system Python), automated WSL/Docker startup, file sync to `/opt/passportvault`, container rebuild and force recreation, document re-verification via `reverify_all.py`, diagnostic harness execution via `diagnose_records.py`, and containerized test execution.

### Fixed
- Windows Tesseract Auto-Discovery & Exception Safety: Added Windows Tesseract installation path auto-discovery in `vault/ocr.py` and wrapped OCR invocations in safe exception blocks catching `TesseractNotFoundError`, preventing worker subprocess crashes on Windows developer machines.
...
- Host-Level Test Compatibility Without Tesseract: Gracefully skipped host-level OCR smoke tests in `tests/test_vault.py` when the Tesseract binary is not installed in the host OS PATH (Tesseract is containerized in Docker), preventing false test failures during updater pre-flight checks on Windows developer machines.
